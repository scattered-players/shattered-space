...
var server = 'https://janus.brokenmirrors.services:8089/janus'
...
function receiveMessage(messageText) {
	...
	message.recvTimestamp = Date.now();
	message.size = messageText.length;
	messages.push(message);
	if(messages.length > 100) {
		messages.shift();
	}

	let oldestStamp = messages[0].recvTimestamp;
	let fps = Math.round(1000* messages.length / (Date.now() - oldestStamp));
	let kbps = Math.round(8 * messages.reduce((acc, message) => acc+message.size, 0) / (Date.now() - oldestStamp))
	$('#datarecv').val(`FPS: ${fps}kbps:${kbps}`);
}
...
function updateStreamsList() {
	...
	streaming.send({"message": body, success: function(result) {
        ...
        if(list[mp]["description"] === 'Shattered Ending'){
